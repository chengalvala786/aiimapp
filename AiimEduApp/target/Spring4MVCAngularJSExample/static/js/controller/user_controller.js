...
App.controller('UserController', ['$scope', 'regService','UserService','$rootScope','$log' ,'$location', 'Upload' , '$base64', '$routeParams',function($scope, regService,UserService,$rootScope,$log, $location, Upload,$base64,$routeParams) {
    ...
    $scope.upload = function (file) {
        Upload.upload({
            url: 'upload',
            data: {file: file, 'email': $rootScope.application.personalInfo.emailId}
        }).then(function (resp) {
...
//Click of review button
	...
	$scope.saveAndExit = function(page) {
			$rootScope.application.pageNo=page;
				...
				if ($rootScope.application.personalInfo.photo!=null){
					var byteF =  $rootScope.application.personalInfo.photo.$ngfDataUrl;
					var encoded= $base64.encode(byteF);
					$rootScope.application.personalInfo.photo = encoded;
				}
				...
				function(data){
					...
					if (data != ""){
						...
						if ($rootScope.application.personalInfo.dob!=null){
							$rootScope.application.personalInfo.dob =new Date($rootScope.application.personalInfo.dob);
						}
						var newRecord = {
								id:  1,
								value: ''
	...
	$scope.createUser = function() {
		...
		if ($scope.userInfoForm.$invalid ) { 
			...
			$scope.isMName = $scope.userInfoForm.middle_name.$error.required;
	...
	$scope.personalInfoNext = function() {
		if ($scope.personalInfoForm.$invalid) { 
			...
			$scope.isMName = $scope.personalInfoForm.middle_name.$error.required;
	...
	$scope.toggleTag = function(tag){
		  var idx = $rootScope.application.additionalInfo.src.selected.tags.indexOf(tag);
		  if (idx > -1){
			  $rootScope.application.additionalInfo.src.selected.tags.splice(idx, 1);
		  }else{
			  $rootScope.application.additionalInfo.src.selected.tags.push(tag);
		  }
		}
